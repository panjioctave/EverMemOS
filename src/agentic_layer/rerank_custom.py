"""
Custom (Self-Deployed) Rerank Service Implementation

Reranking service for self-deployed vLLM or similar services.
"""
...
@dataclass
class CustomRerankConfig:
    """Custom rerank service configuration"""

    api_key: str = "EMPTY"
    base_url: str = "http://localhost:8000/v1/rerank"
    ...
    def __post_init__(self):
        ...
        if self.api_key == "EMPTY":
            self.api_key = os.getenv("CUSTOM_RERANK_API_KEY", "EMPTY")

        if self.base_url == "http://localhost:8000/v1/rerank":
            self.base_url = os.getenv("CUSTOM_RERANK_URL", self.base_url)

        if self.model == "Qwen/Qwen3-Reranker-4B":
            self.model = os.getenv("CUSTOM_RERANK_MODEL", self.model)

        if self.timeout == 30:
            self.timeout = int(os.getenv("CUSTOM_RERANK_TIMEOUT", os.getenv("RERANK_TIMEOUT", "30")))
...
class CustomRerankService(RerankServiceInterface):
    """Custom (vLLM) reranking service implementation"""

    def __init__(self, config: Optional[CustomRerankConfig] = None):
        if config is None:
            config = CustomRerankConfig()

        self.config = config
        self.session: Optional[aiohttp.ClientSession] = None
        self._semaphore = asyncio.Semaphore(config.max_concurrent_requests)
        logger.info(
            f"Initialized CustomRerankService | url={config.base_url} | model={config.model}"
        )
    ...
    async def _send_rerank_request_batch(
        ...
        """Send rerank request batch to custom rerank API (OpenAI-compatible format)"""
                ...
                try:
                    async with self.session.post(url, json=request_data) as response:
                        if response.status == 200:
                            ...
                            logger.error(
                                f"Custom rerank API error {response.status}: {error_text}"
                            )
                    ...
                    logger.error(f"Custom rerank exception: {e}")
                    if attempt < self.config.max_retries - 1:
                        await asyncio.sleep(2**attempt)
                        continue
    ...
    def _parse_response(self, json_body: Dict[str, Any]) -> Dict[str, Any]:
        """Parse custom rerank API response (OpenAI-compatible format)"""
        scores = []
        if "results" in json_body:
            # Sort by index to ensure correct order
            results = sorted(json_body["results"], key=lambda x: x.get("index", 0))
    ...
    async def rerank_memories(
        ...
        """Rerank memories using custom vLLM API"""
